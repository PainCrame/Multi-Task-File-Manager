...
{
    DIR *d;
    if(opendir("MultiTaskFM") == NULL)
        if(mkdir("MultiTaskFM") != 0)
            tell_error(__MAKE_FOLDER__ERROR__, "MultiTaskFM");
    
    if(chdir("MultiTaskFM") != 0)
        tell_error(__FOLDER__OPENNING__ERROR__, "MultiTaskFM");
}
...
void fcopy(char *file_name, char *new_file_name)
{
    ...
    {
        fprintf(stderr, "%s opening folder failed.", file_name);
        exit(EXIT_FAILURE);
    }
    ...
    {
        fprintf(stderr, "Failed to open the save log file");
        exit(EXIT_FAILURE);
    }
