...
void desinstall(char *file_name)
{
    ...
    remove(file_name);
    printf(GREEN "\nDesinstallation" RESET " de" BLUE " %s "RESET" r%dussi !", file_name, é);
}
...
void process_uninstall_file(void)
{
    ...
    printf("\n Entrez le nom des fichiers %d supprimer, avec leur "BLUE"extension"RESET" (par exemple : document"YELLOW".txt"RESET" ou image"YELLOW".png"RESET") (s%dpar%d par des "BOLDWHITE"virgules"RESET") : ", à, é, é);
